...
#安装辅助工具  phpredisadmin adminMogno
RUN cd /usr/src \
...
#环境变量设置
ENV PATH /usr/local/php/bin:/etc/init.d:/usr/local/redis/bin:/usr/local/go/bin:/root/go/bin:/usr/local/node/bin:/usr/local/mongodb/bin:usr/local/java/bin:/usr/local/maven/bin:$PATH
ENV GOPATH /root/go
