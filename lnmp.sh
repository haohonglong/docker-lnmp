...
sudo apt-get update
#基础环境配置
sudo apt-get install vim wget git net-tools -y \
    && sudo apt-get install epel-release -y \
    && sudo apt-get update -y \
    && sudo apt-get -y install pcre pcre-devel zlib zlib-devel openssl openssl-devel libxml2 libxml2-devel libjpeg libjpeg-devel \
    ...
    && sudo apt-get install vixie-cron crontabs -y \
    ...
    && sudo apt-get clean all && rm -rf /var/cache/sudo apt-get/*
